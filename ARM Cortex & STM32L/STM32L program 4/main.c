...
uint32_t GPIOA_IDR_value;
uint32_t GPIOB_ODR_value;
int State = 0;
int blink = 0;
...
void EXTI0_IRQHandler(void)
{
  ...
  // INSERT BUTTON DETECTION CODE HERE
  GPIOA_IDR_value = ptr_PORTA->GPIOx_IDR;
  if (GPIOA_IDR_value & GPIO_Pin_0)
  {
    State =+ 1;
    if(State == 4)
            State = 0;
